...
contract Lottery is ERC20, Ownable {
    ...
    function getWinnerDetails() public view returns (uint, address) {
    require(winner != address(0), "No winner selected yet");
    uint winningTicket;
    for (uint i = 0; i < purchasedTickets.length; i++) {
        if (ticketOwner[purchasedTickets[i]] == winner) {
            winningTicket = purchasedTickets[i];
            break;
        }
    }
    return (winningTicket, winner);
}
